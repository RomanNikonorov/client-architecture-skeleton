...
- Для создания конкретного `RestClient` используется shared bean `outboundRestClientBuilder` из technical package `http`: configuration method принимает его через `@Qualifier`, вызывает `clone()`, затем задает `baseUrl` и `requestFactory`. Статический `RestClient.builder()` в infrastructure configuration конкретной интеграции не используется, чтобы не терять observability и tracing.

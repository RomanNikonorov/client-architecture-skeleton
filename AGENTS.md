...
- `client` - агрегация клиентского профиля и внешних сигналов.
...
- `infrastructure.concurrent` - реализация fan-out на virtual threads.
...
```text
src/main/java/me/nikonorov/clients/<context>/api/rest
```
...
Для новой REST-точки добавь тест адаптера рядом с существующим примером в `src/test/java/me/nikonorov/clients/<context>/api/rest`.
...
```proto
option java_package = "me.nikonorov.clients.<context>.api.grpc.generated";
```

gRPC service размещай в:

```text
src/main/java/me/nikonorov/clients/<context>/api/grpc
```
...
Для новой gRPC-точки добавь тест адаптера в `src/test/java/me/nikonorov/clients/<context>/api/grpc`.
...
Shared technical API fan-out находится отдельно в `application.fanout`.
...
```text
src/main/java/me/nikonorov/clients/<context>/infrastructure/db
```
...
```proto
option java_package = "me.nikonorov.clients.<context>.infrastructure.grpc.generated";
```

Исходящие gRPC adapters размещай в:

```text
src/main/java/me/nikonorov/clients/<context>/infrastructure/grpc
```
...
```text
src/main/java/me/nikonorov/clients/<context>/infrastructure/rest
```
...
- входящий API: `me.nikonorov.clients.<context>.api.grpc.generated`;
- исходящие внешние системы: `me.nikonorov.clients.<context>.infrastructure.grpc.generated`.

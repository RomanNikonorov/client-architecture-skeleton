...
Это интеграционный микросервис на Spring Boot 4 и Java 25. Проект показывает масштабируемую архитектуру с bounded contexts, которые можно вынести в отдельные сервисы, и с повторяемыми слоями для REST/gRPC endpoint и внешних интеграций по gRPC/REST.
...
- `<context>.api.rest` - входные REST-адаптеры.
- `<context>.api.grpc` - входные gRPC-адаптеры.
- `<context>.application.usecase` - сценарии использования, command/result records и бизнес-оркестрация.
- `<context>.application.port` - прикладные порты к внешним системам.
- `<context>.domain` - доменные типы и контракты репозиториев.
- `<context>.infrastructure` - JPA, исходящие gRPC/REST адаптеры, техническая конфигурация.

Shared technical packages:

- `fanout` - технический API для ограниченного fan-out внутри сценариев использования.
- `concurrent` - реализация fan-out на virtual threads.
- `internal` - детали реализации, которые не должны использоваться другими модулями.

Правила зависимостей:

- `<context>.api.*` не зависит от `<context>.infrastructure.*`.
- `<context>.application.*` не зависит от `<context>.api.*`, `<context>.infrastructure.*`, REST, gRPC, JPA и сгенерированных классов.
- `<context>.domain.*` не зависит от Spring, JPA, REST, gRPC и infrastructure.
- Один bounded context не импортирует application/domain/infrastructure/API типы другого bounded context.
- Исходящие интеграции доступны прикладному слою только через порты из `<context>.application.port`.
...
```text
src/main/java/me/nikonorov/<context>/api/rest
```
...
Для новой REST-точки добавь тест адаптера рядом с существующим примером в `src/test/java/me/nikonorov/<context>/api/rest`.
...
```proto
option java_package = "me.nikonorov.<context>.api.grpc.generated";
```

gRPC service размещай в:

```text
src/main/java/me/nikonorov/<context>/api/grpc
```
...
Для новой gRPC-точки добавь тест адаптера в `src/test/java/me/nikonorov/<context>/api/grpc`.

## Application слой

Прикладной слой каждого bounded context разделен на два подпакета:

- `<context>.application.usecase` - бизнес-сценарии, command/result records, orchestration;
- `<context>.application.port` - интерфейсы исходящих портов.

Shared technical API fan-out находится отдельно в `fanout`.
...
- прикладной технический API: `fanout.FanOutExecutor`;
...
```text
src/main/java/me/nikonorov/<context>/infrastructure/db
```
...
```proto
option java_package = "me.nikonorov.<context>.infrastructure.grpc.generated";
```

Исходящие gRPC adapters размещай в:

```text
src/main/java/me/nikonorov/<context>/infrastructure/grpc
```
...
```text
src/main/java/me/nikonorov/<context>/infrastructure/rest
```
...
```text
src/test/java/me/nikonorov/architecture/ArchitectureRulesTest.java
src/test/java/me/nikonorov/ModulithStructureTest.java
```
...
- входящий API: `me.nikonorov.<context>.api.grpc.generated`;
- исходящие внешние системы: `me.nikonorov.<context>.infrastructure.grpc.generated`.
...
- сценарии использования применяют `fanout.FanOutExecutor`, если нужна параллельность;

...
Именованный `RestClient` создается через `mutate()` от shared bean
`outboundRestClient`, чтобы сохранить observability и tracing:
...
3. Добавить именованные beans `RestClient` и request factory в
   ...
   `outboundRestClient.mutate()`, а не через статический `RestClient.builder()`.

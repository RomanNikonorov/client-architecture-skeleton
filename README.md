...
который предоставляет Spring Boot. Builder настраивается через shared helper:
он добавляет `baseUrl`, request factory и interceptors конкретной интеграции,
...
3. Добавить именованные beans `RestClient` и request factory в
   ...
   `PooledRestClientRequestFactory.restClientBuilder(...)`, а не через
   статический `RestClient.builder()`.
4. Создать logging interceptor в infrastructure configuration приложения и
   передать его в helper вместе с другими interceptors, если они нужны.
5. В adapter внедрить `RestClient` через `@Qualifier`.
6. Покрыть adapter mapping/fallback behavior тестом.
